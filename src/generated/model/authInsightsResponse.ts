/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Volcano Hosting API
 * Public API for Volcano Hosting clients, SDKs, and CLI tooling (Port 8000).
 *
 * This specification intentionally excludes first-party/internal APIs. See api/openapi-internal.yaml for non-public internal and Builder operations.
 * OpenAPI spec version: 3.0.0
 */
import type { AuthInsightsSeriesPoint } from './authInsightsSeriesPoint';
import type { AuthInsightsSummary } from './authInsightsSummary';
import type { AuthInsightsWindow } from './authInsightsWindow';

export interface AuthInsightsResponse {
  project_id: string;
  observed_at: string;
  window: AuthInsightsWindow;
  summary: AuthInsightsSummary;
  series: AuthInsightsSeriesPoint[];
}
