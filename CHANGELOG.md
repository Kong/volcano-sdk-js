...
- A checked-in, deterministic Fetch client generated by `@hey-api/openapi-ts`
  from the 169-operation OpenAPI contract.
- `@volcano.dev/sdk/api`, exporting `createApiClient`, every generated operation,
...
- Database, auth, storage, function-resolution, and Next.js helpers now use
...
- Code generation and build run on Node.js 22; the published package continues
  to support Node.js 20 and dual ESM/CommonJS consumers.

### Removed

- The `VolcanoAuth` constructor, direct management/log wrappers, mutable database
  selection, handwritten OpenAPI declaration maps, and generic internal fetch
  helpers. Use `createVolcanoClient()` and generated API operations instead.
