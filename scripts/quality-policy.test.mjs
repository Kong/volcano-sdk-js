import assert from 'node:assert/strict';
import { readFile } from 'node:fs/promises';
import test from 'node:test';

const manifest = JSON.parse(await readFile(new URL('../package.json', import.meta.url), 'utf8'));
const requiredTasks = {
  quality: 'pnpm quality:policy && pnpm quality:checks && pnpm mutation:full',
  'quality:checks':
    'pnpm run audit && pnpm lint && pnpm check:unused && pnpm check:openapi && pnpm test:types && pnpm test:tooling && node scripts/prepare-test-reports.mjs && pnpm test --ci --json --outputFile=reports/unit.json && pnpm test:typed-runtime && pnpm test:quickstart && pnpm test:examples',
  'quality:policy': 'node --test scripts/quality-policy.test.mjs',
  audit: 'node scripts/audit-dependencies.mjs',
  lint: 'pnpm build && eslint . --max-warnings=0 && pnpm format:check',
  build: 'rollup -c rollup.config.mjs && node scripts/copy-types.mjs',
  'format:check': 'prettier . --config prettier.config.cjs --check',
  'check:unused':
    'knip --files && knip --production --files && knip --include exports && knip --include dependencies,unlisted,binaries',
  'check:openapi': 'pnpm generate:openapi && node scripts/check-openapi.mjs',
  'generate:openapi':
    'node scripts/clean-openapi.mjs && orval --config orval.config.mjs && openapi-typescript openapi/openapi.yaml --default-non-nullable false -o src/generated/openapi.d.ts && prettier src/generated/openapi.d.ts --write && tsc -p tsconfig.generated.json',
  'test:types':
    'pnpm build && tsc --noEmit -p tsconfig.json && tsc --noEmit -p tsconfig.consumer.json',
  'test:tooling': 'node --test --test-concurrency=1 scripts/*.test.mjs',
  test: 'pnpm build && jest',
  'test:typed-runtime': 'pnpm build && jest --config jest.typed.config.cjs',
  'test:quickstart':
    'pnpm test:package && pnpm test:types:package && node scripts/test-package-quickstart.mjs reports/volcano-sdk.tgz',
  'test:package':
    'pnpm pack --out reports/volcano-sdk.tgz && publint reports/volcano-sdk.tgz --strict && attw reports/volcano-sdk.tgz',
  'test:types:package': 'tsc --noEmit -p tsconfig.consumer.json',
  'test:examples':
    'pnpm build && NEXT_PUBLIC_VOLCANO_API_URL=http://127.0.0.1:8787 NEXT_PUBLIC_VOLCANO_ANON_KEY=example-test-key NEXT_PUBLIC_VOLCANO_DATABASE_NAME=app pnpm --dir examples/nextjs-notes-app build',
  'mutation:full': 'node scripts/run-mutation.mjs',
};

function requireTasks(scripts) {
  for (const [name, command] of Object.entries(requiredTasks)) {
    assert.equal(scripts[name], command, `${name} must run its mandatory native check`);
  }
}

test('canonical quality tasks include every mandatory check', () => {
  requireTasks(manifest.scripts);
});

test('every mandatory task is checked independently of its own execution', () => {
  for (const name of Object.keys(requiredTasks)) {
    assert.throws(() => requireTasks({ ...manifest.scripts, [name]: 'node --version' }));
  }
});
