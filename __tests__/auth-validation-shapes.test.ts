...
test('preserves profiles without the optional public status', () => {
  const profile = { id: 'user-1', email: 'user@example.com' };
  expect(validateAuthUser(profile)).toBeNull();
  expect(validateOAuthSession({ access_token: 'access', user: profile })).toBeNull();
  expect(validateAuthUser({ ...profile, status: undefined })).toEqual(
    new TypeError('Auth user status must be active, banned, or deleted'),
  );
});
